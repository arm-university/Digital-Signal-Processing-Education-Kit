// stm32f7_dimpulse_recon48_IIR_intr.c

#include "stm32f7_wm8994_init.h"
#include "stm32f7_display.h"
#include "recon14julyiir.h"

#define SOURCE_FILE_NAME "stm32f7_dimpulse_recon48_IIR_intr.c"

#define LOOPLENGTH 128
#define OVERSAMPLING_RATIO 6
#define DIMPULSE_AMPLITUDE 30000.0
...
int16_t dimpulse_table[LOOPLENGTH] = {0};
int16_t dimpulse_ptr = 0;
...
// global variables tx_sample_L and tx_sample_R
  ...
  {
    count = 0;
    xn = (float32_t)(dimpulse_table[dimpulse_ptr]);
    dimpulse_ptr = (dimpulse_ptr+1) % LOOPLENGTH;
// GPIO pin transition coincides with non-zero sample value
    if (dimpulse_ptr == 0) BSP_LED_Toggle(LED1);
   }
  else
  {
    xn = 0.0;
...
{  
  ...
  dimpulse_table[0] = DIMPULSE_AMPLITUDE;
  k = 0;
